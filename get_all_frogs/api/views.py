...
from get_all_frogs.models import (
    Zabka, StoreComment , User , Achievements , AssignedAcievments , VisitedZabkas
)
from .serializers import (
    ZabkaSerializer, CommSerializer , UserSerializer , AchievementSerializer , AssignedSerializer , VisitedSerializer
)
...
class ZabkaLista(BaseAuth,generics.ListAPIView):
    queryset = Zabka.objects.all()
    serializer_class = ZabkaSerializer

class ZabkaOneShop(BaseAuth,generics.RetrieveAPIView):
    queryset = Zabka.objects.all()
    serializer_class = ZabkaSerializer
    lookup_field = 'id'


class CommentsByStore(BaseAuth,generics.ListCreateAPIView):
...
class CommentsList(BaseAuth,generics.ListCreateAPIView):
    serializer_class = CommSerializer
    def perform_create(self, serializer):
...
class CommentsByParent(BaseAuth,generics.ListCreateAPIView):
    serializer_class = CommSerializer
    def perform_create(self, serializer):
...
class UserList(BaseAuth,generics.ListAPIView):
    serializer_class = UserSerializer
    def get_queryset(self):
        return User.objects.filter(is_superuser=False)


class UserDetail(BaseAuth,generics.ListAPIView):
    serializer_class = UserSerializer
    def get_queryset(self):
        user_id = self.kwargs['user_id']
        return User.objects.filter(id=user_id,is_superuser=False)

class AchievementList(BaseAuth,generics.ListAPIView):
    serializer_class = AchievementSerializer
    queryset = Achievements.objects.all()


class AssignedList(BaseAuth,generics.ListAPIView):
    serializer_class = AssignedSerializer
    queryset = AssignedAcievments.objects.all()

class AssignedListByAchievment(BaseAuth,generics.ListAPIView):
    serializer_class = AssignedSerializer
...
class AssignedListByUser(BaseAuth,generics.ListAPIView):
    serializer_class = AssignedSerializer
